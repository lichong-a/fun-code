...
if [ ! -d "$CODE_HOME"/jdk-11 ]; then
  ...
  echo "正在解压jdk..."
  tar -zxf "$CODE_HOME"/source/openjdk-11+28_linux-x64_bin.tar.gz -C "$CODE_HOME"
  echo "export CODE_JAVA_HOME=${CODE_HOME}/jdk-11" >>/root/.bashrc
  export CODE_JAVA_HOME=${CODE_HOME}/jdk-11
  source /root/.bashrc
